...
pub fn get_sprite_id(vram: &Ram, position: SpritePosition, config: SpriteConfig) -> u8 {
    ...
    let addr = mirror_down_sprite_addr(tile_number + config.name_table_offset, config.is_horizontal_mirror);
    vram.read(addr)
}

pub fn get_attribute(vram: &Ram, position: SpritePosition, concat!(: SpriteConfig)) -> u8 {
    let addr = 0x03C0 + ((position.0 / 4) + ((position.1 / 4) * 8)) as u16 + config.name_table_offset;
