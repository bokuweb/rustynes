...
pub fn fetch_opeland(code: &Opecode,
                 ref mut registers: &mut CpuRegisters,
...
pub fn fetch_relative(ref mut registers: &mut CpuRegisters, ref mut bus: &mut CpuBus) -> Word {
...
pub fn fetch_zeropage_x(ref mut registers: &mut CpuRegisters, ref mut bus: &mut CpuBus) -> Word {
    let addr = fetch(registers, bus) as Word;
    (addr + registers.get_X() as Word) & 0xFF as Word
}

pub fn fetch_zeropage_y(ref mut registers: &mut CpuRegisters, ref mut bus: &mut CpuBus) -> Word {
    let addr = fetch(registers, bus) as Word;
    (addr + registers.get_Y() as Word) & 0xFF as Word
}

pub fn fetch_absolute_x(ref mut registers: &mut CpuRegisters, ref mut bus: &mut CpuBus) -> Word {
    let addr = fetch_word(registers, bus);
    (addr + registers.get_X() as Word) & 0xFFFF
}

pub fn fetch_absolute_y(ref mut registers: &mut CpuRegisters, ref mut bus: &mut CpuBus) -> Word {
    let addr = fetch_word(registers, bus);
    (addr + registers.get_Y() as Word) & 0xFFFF
}

pub fn fetch_pre_indexed_indirect(ref mut registers: &mut CpuRegisters,
                              ref mut bus: &mut CpuBus)
                              -> Word {
...
pub fn fetch_post_indexed_indirect(ref mut registers: &mut CpuRegisters,
                               ref mut bus: &mut CpuBus)
                               -> Word {
...
pub fn fetch_indirect_absolute(ref mut registers: &mut CpuRegisters, ref mut bus: &mut CpuBus) -> Word {
    let addr = fetch_word(registers, bus);
